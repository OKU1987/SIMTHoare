...
Section SIMT_Definition.
  ...
  Fixpoint E_under_state (s:state) e : {ffun T -> int} :=
    match e with
      | tid => [ffun i : T => i%:Z]
      | ntid => [ffun _ => N%:Z]
      | var n (local_variable lv) es =>
        [ffun i => s.1 n lv i (tuple_of_Vector (Vector.map (fun e => s[[e]](i)) es))]
      | var n (shared_variable sv) es =>
        [ffun i => s.2 n sv (tuple_of_Vector (Vector.map (fun e => s[[e]](i)) es))]
      | func n f es =>
        [ffun i => f (tuple_of_Vector (Vector.map (fun e => s[[e]](i)) es))]
    end
  ...
  Definition mask_of (m : {ffun T -> int}) : mask := [set i | bool_of_int (m i)].

  Lemma setI_e_and :
    forall m z,
       (mask_of m) :&: (mask_of z) = mask_of [ffun i => e_and [tuple m i; z i]].
  Proof.
    move => m z.
    apply setP => i.
    rewrite /mask_of/e_and -setIdE 3!in_set ffunE int_of_boolK // .
  Qed.

  Lemma setD_e_and_neg :
    forall m z,
      (mask_of m) :\: (mask_of z) = mask_of [ffun i => e_and [tuple m i; e_neg [tuple z i]]].
  Proof.
    move => m z.
    apply setP => i.
    rewrite /mask_of/e_and/e_neg/= setDE -setIdE 4!in_set ffunE 2!int_of_boolK // .
  Qed.
  ...
  Definition all (e : {ffun T -> int}) := forall i : T, e i != 0.
  Definition none (e : {ffun T -> int}) := forall i : T, e i == 0.

  Definition lassign (s : state) n (x' : T -> n.-tuple int -> int) (m : {ffun T -> int}) (x : LV n) (es : n.-tuple E) (e : E) : Prop :=
  ...
  Definition sassign s n (x' : n.-tuple int -> int) (m : {ffun T -> int}) (x : SV n) (es : n.-tuple E) (e : E) : Prop :=
  ...
  Definition assign rho n x' (m : {ffun T -> int}) (x : V n) (es : n.-tuple _) (e : _) :=
  ...
  Inductive Hoare_proof : assertion -> {ffun T -> int} -> program -> assertion -> Prop :=
  ...
  | H_If : forall (phi : assertion) psi chi e (m : {ffun T -> int}) P Q,
             (forall z : {ffun T -> int},
                Hoare_proof
                  (fun s => (forall i, s[[e]](i) = z i) /\ phi s)
                  [ffun i => e_and [tuple m i; z i]] P (psi z)) ->
             (forall z, Hoare_proof (psi z) [ffun i => e_and [tuple m i; e_neg [tuple z i]]] Q chi) ->
             Hoare_proof phi m (IFB e THEN P ELSE Q) chi
  | H_While : forall (phi : assertion) (m : {ffun T -> int}) e P,
                (forall z : {ffun T -> int},
                   Hoare_proof (fun s => (forall i, s[[e]](i) = z i) /\ phi s) [ffun i => e_and [tuple m i; z i]] P phi) ->
                Hoare_proof phi m (WHILE e DO P) (fun s => phi s /\ none [ffun i : T => e_and [tuple m i; s[[e]](i)]]).

  Lemma lem_1 : forall s s' n (x : V n) es e (m : {ffun T -> int}),
  ...
  Proof.
    ...
    move=> i.
    move: (H _ _ _ H0 i) => {H}.
    rewrite 4!in_set/bool_of_int ?ffunE in H0|-*.
    case: (i \in mu) => // .
    case: (s[[e]](i)) => // .
    case => // .
  ...
  Proof.
    ...
    rewrite ?in_set ?ffunE in H0_,H0_0|-*.
    clear.
  ...
  Lemma Soundness_while :
    forall phi e p (m : {ffun T -> int}),
      (forall z : {ffun T -> int},
         regular p ->
         forall s s' : state,
           (forall i : T, s [[e ]]( i) = z i) /\ phi s ->
           eval p (mask_of [ffun i0 : T => e_and [tuple m i0; z i0]]) s
                  s' -> phi s') ->
      regular (WHILE e DO p) ->
      forall s s' m',
        ...
        phi s ->
        phi s' /\ none [ffun i : T => e_and [tuple m' i; s' [[e ]]( i)]].
  Proof.
    ...
    { symmetry in HeqMu. apply lem_5_2 in HeqMu.
      ...
      rewrite ffunE (elimT eqP (HeqMu i)). destruct (s[[e]](i)); reflexivity. }
    { subst. clear IHeval1.
      inversion H0; subst.
      move: (H (E_under_state s e) H6 s s' (conj (fun i => refl_equal _) H3)); intros.
      clear H HeqW.
      rewrite 2?setI_e_and in IHeval2.
      rewrite {2}/mask_of {3}/mask_of in H2.
      rewrite H2 in H1,H1_,IHeval2.
      rewrite -setI_e_and in H1.
      move: (IHeval2 erefl H0 [ffun i => e_and [tuple m' i; s[[e]](i)]] erefl).
      move: (elimT setIidPr (lem_2 _ _ H5 _ _ _ H1_)).
      ...
      rewrite ?ffunE/e_and/= int_of_boolK.
      rewrite /mask_of.
      move/setP => H8.
      move: (H8 i) => {H8}.
      rewrite 4!in_set.
  ...
  Theorem Soundness : forall phi (m : {ffun T -> int}) p psi,
                        Hoare_proof phi m p psi ->
                        regular p ->
  ...
  Theorem Relative_Completeness' : forall phi (m : {ffun T -> int}) p psi,
                                     regular p ->
                                     hoare_quadruple phi m p psi ->
  ...
  Proof.
    ...
    { eapply H_If; intros.
      { eapply H_Conseq_pre.
        { inversion H; subst.
          eapply IHp1 with
          (phi :=
             (fun s => (forall i, s[[e]](i) = z i) /\
                       wlp [ffun i => e_and [tuple m i; z i]] p1
                           (wlp [ffun i => e_and [tuple m i; e_neg [tuple z i]]] p2 psi) s)); try assumption.
    ...
    { inversion H; subst.
      remember
        (fun s =>
           exists z : {ffun T -> int},
             wlp [ffun i => e_and [tuple m i; z i]] (WHILE e DO p) psi s /\
             (forall i, s[[e]](i) = z i))
        as wp.
      assert (forall z : {ffun T -> int},
                Hoare_proof (fun s => (forall i, s[[e]](i) = z i) /\ wp s)
                            [ffun i => e_and [tuple m i; z i]] p wp).
      { intros.
        unfold hoare_quadruple in H0.
        eapply H_Conseq_pre.
        { eapply IHp with (phi:=fun s => (forall i, s[[e]](i) = z i) /\ wp s);
          try assumption; subst.
          unfold hoare_quadruple. intros.
          destruct H1 as [H1 [z' [H1' H1'']]].
        ...
        { intros.
          ...
          rewrite -H2'.
          rewrite -H1 in H5.
          move: (lem_2 _ _ H3 _ _ _ H5) => lem_2_H3.
      ...
      { econstructor.
        ...
        { intros.
          ...
          rewrite setI_e_and in H5.
          remember (WHILE e DO p) as W.
          remember (mask_of [ffun i => e_and [tuple m i; s[[e]](i)]]) as mu.
        ...
        { cbv beta.
          ...
          unfold wlp in H2.
          eapply H2.
          assert (mask_of [ffun i => e_and [tuple m i; z i]] = empty).
          { apply setP=> i.
            apply setP in H5.
            move: (H5 i).
            rewrite ?in_set 2!ffunE -(H2' i) => // . }
          rewrite H6.
          econstructor. }}}
  Qed.
